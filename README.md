...
**Stuck positions vanish from `/positions`/`/portfolio`, `/info` can go silent, alerts can duplicate across a restart, and the underlying close-order rejection is still live** — a test-mode session showed `/positions` reply "No open positions" and `/info trade_id=N` reply with nothing at all immediately after a "MANUAL ACTION REQUIRED" alert for that same trade, and the same alert firing twice across a service restart. Root-caused ([BOT_TODO.md Phase 22](BOT_TODO.md#phase-22--stuck-position-visibility-silent-telegram-failures-and-close-order-price-rejections) / [BOT_PLAN.md Phase 22](BOT_PLAN.md#phase-22--stuck-position-visibility-silent-telegram-failures-and-close-order-price-rejections), planned, not yet fixed): `get_open_trades()` (used by the Telegram handlers) excludes `close_status='close_stuck'` rows, hiding exactly the positions an operator needs to see after an alert; `handle_info` has no error handling and the Telegram listener registers no global error handler, so an exception (e.g. a divide-by-zero when a stuck position's cost basis is `0`) fails completely silently instead of replying; and — the more serious finding — the monitor loop's own read path (`_load_all_open_positions`/`load_calendar_state`) was never updated to exclude `close_stuck` positions the way `get_open_trades()` was, so the engine keeps retrying a stuck position's close on every tick forever and keeps re-triggering the stuck-notification path, which a restart's cleared in-memory dedup set then lets through as a duplicate alert. The close/roll order rejection that gets positions stuck in the first place is also still live: `execution/executor.py` computes close/roll prices as a synthetic `mid * 1.02`/`mid * 0.98` instead of a real quote, with tick-size fetch failures silently swallowed, reproducing the same deterministic `-32602 Invalid params` rejection Phase 18 was believed to have fixed. A related, smaller-scale bug was also found in the paper-mode run: trades entered with a 1-day near leg were rolled or closed roughly a minute after entry, because `ROLL_TRIGGER_DAYS` (`2`) makes any freshly-opened 1-day-near position roll-eligible from its very first monitor tick regardless of real elapsed time, and the roll-candidate matching had no check that a new near leg's expiry actually precedes the position's own far leg — one instance rolled into a same-expiry near/far pair, collapsing its spread value to `$0.00` and instantly tripping a large stop-loss. This is the same "degenerate spread fools stop/TP" failure family Phase 21 fixed on the entry-ranking path, recurring here on the roll path.
