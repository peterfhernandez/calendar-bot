...
**Healthy positions liquidated on a single failed roll, doomed-by-construction short-DTE entries, mark-vs-executable valuation gaps, and untracked exchange inventory from failed legged entries** — analysis of the 2026-07-22 → 2026-07-23 test-mode run (trades 14–18) and the 2026-07-22 paper-mode close of trade 208 surfaced five further issues, planned for fixing in [BOT_TODO.md Phase 26](BOT_TODO.md#phase-26--legged-entry-fill-safety-roll-failure-resilience-entry-tenor-alignment-and-executable-value-monitoring) / [BOT_PLAN.md Phase 26](BOT_PLAN.md#phase-26--legged-entry-fill-safety-roll-failure-resilience-entry-tenor-alignment-and-executable-value-monitoring): (1) a roll failure closes the position on the **first** failed attempt (`POSITION_FAILURE_RETRY_CAP` never applies on that path), and the roll candidate search inherits entry-grade filters plus a top-match-only selection that can return the currently-held near leg — three healthy positions (110%/101%/98% of debit) were liquidated on one bad scan tick for −162.42/−61.59/−89.65, and paper trade 208 became structurally unrollable once spot drifted its strike past the 15% moneyness cap; the fix makes roll failures retryable up to the cap, iterates all candidates excluding the held near, relaxes entry-only filters for rolls, and labels such closes honestly instead of `Loss (Auto Stop)`. (2) Entries can open with a near leg already inside `ROLL_TRIGGER_DAYS` (a 2-DTE near matched near-target 1 via the ±3-day tolerance), guaranteeing an immediate roll attempt — a new `MIN_NEAR_DTE_AT_ENTRY` gate rejects them. (3) Stop/TP valuation uses marks while a real close crosses the spread — monitor said 98–110% of debit, closes recovered ~5–10% of marked value; a new executable-value basis (`SPREAD_VALUE_BASIS`) prices decisions at far-bid − near-ask, and paper-mode closes will realise mark-to-market P&L instead of closing at entry value (`gross_pnl=0.00`). (4) Cancelling a timed-out entry leg ignores partial fills — cancels only kill the unfilled remainder, and three failed ETH-1750 legged entries left 13 naked short puts plus an unrecorded 5×5 calendar on the exchange with no DB record; the fix reads `filled_amount` after every cancel and flattens the filled portion immediately. (5) The post-fill slippage check is symmetric (rejects price *improvement* — the only deviation a limit order can produce) and fires after both legs have filled with no unwind, abandoning executed spreads unrecorded; the fix makes it directional and mandates unwind-or-record. Until Phase 26 lands, the orphaned 1750 inventory must be cleared manually on test.deribit.com.
