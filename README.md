...
Two further findings from the same analysis are documented but **not yet fixed** — see [BOT_TODO.md Phase 27](BOT_TODO.md#phase-27--option-amount-step-blocks-all-btc-entries) sub-phases 27d–27e: `CalendarExecutor._run`'s blocking `.result()` freezes the shared asyncio event loop (feed, listener, and monitor) for 47–57s per failed entry; and the liquidity gate compares `ask_size` against a constant rather than against the sized quantity.
