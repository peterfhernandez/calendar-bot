...
**Silent WebSocket data blackout causes the bot to idle indefinitely** — the feed's existing reconnect logic only triggers on TCP/ping failures. If Deribit stops pushing ticker data while keeping the connection open (observed 2026-07-19: feed subscribed at 07:55 AEST, no reconnect, all 532 cached snapshots stale by ~08:00, zero candidates on every scan for 7+ hours), no recovery is triggered and the bot sits idle until manually restarted. Planned fix: [Phase 23 — Feed Freshness Watchdog](BOT_TODO.md#phase-23--feed-freshness-watchdog) adds a background task inside `DeribitFeed` that tracks the last ticker timestamp and closes the WS (triggering the existing reconnect loop) if no update arrives within `FEED_WATCHDOG_TIMEOUT_SEC` (default 120s).

**Reconcile mismatch warnings from `close_stuck` positions are not actionable** — when the bot marks a position `close_stuck` after retry exhaustion (e.g. trades 6–9, `BTC-15JUL26-*` options), the Deribit position stays open and the portfolio tracker detects a margin mismatch every scan cycle. The warning does not name which Deribit instruments are causing it, there is no auto-recovery when the operator manually closes those legs on the exchange, and there is no Telegram command to inspect the live Deribit position list. Planned fix: [Phase 24 — Reconcile Mismatch Remediation](BOT_TODO.md#phase-24--reconcile-mismatch-remediation-for-close_stuck-positions) names the culprit instruments in the warning, auto-reconciles `close_stuck` DB trades once both legs are confirmed gone from Deribit, and adds a `/deribit_positions` Telegram command.
