...
**No BTC order could reach the exchange — an option's `contract_size` was being used as its order-amount step** — investigating two `LegRiskError: Far leg timeout` alerts (2026-07-26 20:03, 2026-07-27 06:18) surfaced a more damaging defect in the same log window: every BTC entry aborted before submission with `AMOUNT GATE: entry aborted — amount 0.2 below exchange minimum for BTC-31JUL26-64000-P`, even though the real BTC option minimum is `0.1`. `execution/executor.py::_fetch_amount_info` derived the amount step as `contract_size or min_trade_amount`, but on Deribit an **option's** `contract_size` is the underlying per contract (always `1`) and is not the order-amount granularity — that is `min_trade_amount` (BTC `0.1`, ETH `1`). Reading it as the step quantised BTC orders to whole BTC, so a sizer-approved `0.2` floored to `0.0` and was rejected. ETH concealed the bug entirely (both fields are `1` there, so the readings agree), which is why every observed `LegRiskError` was ETH while BTC simply never traded — and why the existing tests missed it: every fixture used ETH values in which the two fields are indistinguishable. Now fixed ([BOT_TODO.md Phase 27](BOT_TODO.md#phase-27--option-amount-step-blocks-all-btc-entries) / [BOT_PLAN.md Phase 27](BOT_PLAN.md#phase-27--option-amount-step-blocks-all-btc-entries)): options step on `min_trade_amount` while futures/perpetuals keep `contract_size`; a new `_is_option_instrument()` prefers the API `kind` field and falls back to the `-C`/`-P` name suffix so a missing `kind` cannot fall through to the futures reading; and `resolve_amount_info()` is split out of `clamp_amount()` so the rejection message names the minimum *and* the step instead of misreporting a step failure as a minimum failure. Demonstrated offline by `python -m scratch.scratch_option_amount_step`.

**A fully-filled leg was declared a timeout and unwound at a crossed price** — the same analysis found that `_wait_for_fill` (`execution/executor.py`) judged an order on `order_state` alone, which on Deribit can lag the last fill. Order `ETH-82752446337` reported `8.0000` of 8 filled — complete — yet the wait had already raised `OrderTimeoutError`, so the far leg was unwound at a crossed price *and* a `FLATTEN-NEAR` fired against a correctly-filled near leg: a round trip through the spread on both legs for nothing. Compounding it, the poll backoff (1.0s, ×1.5, capped at 5.0s) slept the full interval regardless of the time left, so on a 30s timeout the last state observation landed at t≈28.1s while the loop ran to t≈33.1s — a ~5s window in which a completing fill was invisible (the logged 06:17:58 → 06:18:32 gap is 34s, not the 30s the message claimed, because the message was formatted from the nominal timeout rather than measured time). Now fixed ([BOT_TODO.md Phase 27b](BOT_TODO.md#27b--_wait_for_fill-is-all-or-nothing-and-blind-at-the-deadline) / [BOT_PLAN.md Phase 27](BOT_PLAN.md#phase-27--option-amount-step-blocks-all-btc-entries)): completion is detected by quantity (`filled_amount` vs the submitted amount, threaded through all seven call sites) as well as by state; the sleep is clamped to the time remaining so a poll always lands *at* the deadline before giving up; and `OrderTimeoutError` now carries `filled_amount`/`amount`/`elapsed_sec` and reports true elapsed time plus fill progress. A genuine partial fill still times out and still goes through Phase 26a's flatten path. Demonstrated offline by `python -m scratch.scratch_wait_for_fill`.

**Every entry silently skipped the combo order and used the last-resort path instead** — the same analysis found `RANK approved` → `Falling back to individual legs` taking a consistent ~2s against a 30s `COMBO_FILL_TIMEOUT_SEC`, so the combo was failing outright rather than timing out, on 100% of entries; both failure branches logged at DEBUG under a `WARNING` root level, so the log never said why. Checked against the exchange (`public/get_combos`, identical on test and production), a combo is returned as `{"id": "BTC-CCAL-25DEC26_31JUL26-115000", "state": "active", "legs": [{"instrument_name": …, "amount": -1}, {…, "amount": 1}]}` — against which `_async_enter_spread_combo` had two mismatches: it read the identifier from `combo_id`/`instrument_name` (Deribit returns neither — it is `id`), so every *successful* `create_combo` was discarded as "no combo_id" one RPC round trip after approval; and it sent the sized quantity as the leg amount, when a combo's legs define its *ratio* in small integers and the traded size belongs on the order placed against the combo instrument (a fractional BTC `0.2` ratio is not a valid definition). Now fixed ([BOT_TODO.md Phase 27c](BOT_TODO.md#27c--combo-path-fails-on-every-entry-and-the-reason-is-invisible) / [BOT_PLAN.md Phase 27](BOT_PLAN.md#phase-27--option-amount-step-blocks-all-btc-entries)): `_combo_id_from_result()` reads `id` (accepting the old names as aliases), `_combo_ratio_legs()` emits the 1:1 integer ratio with the sized qty on the combo order, and every combo failure now logs at `WARNING` as `COMBO: <reason> — falling back to individual legs` with the reason threaded into the fallback warning, so a last-resort path can never again be indistinguishable in the log from the primary path working. A third defect on the (never-executed) success path is fixed too: Deribit reports a combo fill as a single net price, and the old fallback recorded the near leg at the net-debit limit and the far leg at *twice* it — figures unrelated to either fill that fed fees and P&L; `_combo_leg_fills()` now uses a per-leg breakdown when present and otherwise derives the far leg so `far − near` equals the net actually filled. Demonstrated offline by `python -m scratch.scratch_combo_path`.

**A failed entry froze the whole bot for 47–57s, and the liquidity gate had already seen the number that made the entry impossible** — the last two findings from the same analysis, both now fixed ([BOT_TODO.md Phase 27d](BOT_TODO.md#27d--blocking-result-freezes-the-shared-event-loop) / [27e](BOT_TODO.md#27e--liquidity-gate-ignores-the-sized-quantity) / [BOT_PLAN.md Phase 27](BOT_PLAN.md#phase-27--option-amount-step-blocks-all-btc-entries)):

- **The shared event loop stalled for the length of every failed entry.** `CalendarExecutor._run` waits on a blocking `.result()`, and `scan_tick()` was awaited inline from `_scan_job` — an `async def` on the one AsyncIOScheduler loop that also runs the WebSocket feed, the Telegram listener, and the collector. The two failed entries blocked it for **57s** and **47s**, each losing a whole monitor tick (`Run time of job "BotLoop._monitor_job" was missed by 0:00:46.679841`) and freezing the feed until every snapshot aged past the 30s cache TTL (`106 stale instrument(s) excluded from ETH chain`) — with `trade_id=22` sitting at 63% of debit against a 50% hard stop inside one of those blind windows. Since the ticks are synchronous by design, no change inside the executor could fix this: `monitor/loop.py::_run_tick()` now dispatches each tick with `asyncio.to_thread` so the loop keeps pumping throughout, while a `_tick_lock` keeps scan and monitor ticks serialized (engine counters are not safe for concurrent ticks). A long entry therefore still *delays* the next monitor tick, but that tick now reads a live chain cache instead of a stale one and the feed and listener never stall. `TICK_OFFLOAD_ENABLED` (default on) restores inline execution, `TICK_SLOW_WARN_SEC` (default 30s) names any slow tick in the log, and the executor's in-loop branch now logs a warning instead of blocking silently. Demonstrated offline by `python -m scratch.scratch_tick_offload` (0 heartbeats inline vs ~146 offloaded across a 1.5s block).
- **The liquidity gate sized against a constant, not the order.** It compared each leg's quoted size to `MIN_LEG_BID_SIZE`/`MIN_LEG_ASK_SIZE` (both `1`) and never to `candidate.qty`, which is set immediately before the gate runs — so a book quoting one contract passed for an order of any size, and qty 29 was approved into a far ask quoting 8, producing the partial fill that timed out and had to be flattened. A leg quoting *nothing* was worse: `if far_snap.ask_size > 0:` skipped the check entirely rather than failing it. The gate now requires `max(MIN_LEG_*_SIZE, qty)` on the side each leg actually hits (near sold into the bid, far bought from the ask), keeps the plain floor on the two sides the entry never touches so it did not become blanket-stricter, rejects zero quoted size, and names the binding constraint (`far-leg ask_size 8.0 < order qty 29.0`). A roll is checked only on the new near leg it sells — holding it to the depth of an untouched far leg would re-create the Phase 26c liquidation-by-inapplicable-gate failure. `REQUIRE_LEG_SIZE_FOR_QTY` and `REQUIRE_LEG_SIZE_DATA` (both default on) gate the two behaviours separately. Every `_make_snap` fixture in the test suite quoted zero size, which is why nothing caught this — the same blind spot as the all-ETH amount fixtures above. Demonstrated offline by `python -m scratch.scratch_liquidity_gate_qty`.
