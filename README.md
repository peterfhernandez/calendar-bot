...
Three further findings from the same analysis are documented but **not yet fixed** — see [BOT_TODO.md Phase 27](BOT_TODO.md#phase-27--option-amount-step-blocks-all-btc-entries) sub-phases 27c–27e: the combo path fails ~2s after RANK approval on every entry, forcing the individual-leg fallback that BOT_PLAN §5 designates as a last resort; `CalendarExecutor._run`'s blocking `.result()` freezes the shared asyncio event loop (feed, listener, and monitor) for 47–57s per failed entry; and the liquidity gate compares `ask_size` against a constant rather than against the sized quantity.
