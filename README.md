...
- **Log hygiene** (`monitor/loop.py` `configure_logging()`) — `httpx`, `httpcore`, and `telegram` loggers are set to WARNING so the high-frequency `getUpdates` polling calls do not flood the log. A `_SecretRedactor` filter on the root logger replaces any occurrence of `TELEGRAM_TOKEN` or `TELEGRAM_CHAT` in log output with `<redacted>`, preventing credentials from appearing in log files or the console.
