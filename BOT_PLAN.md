...
**Status:** Complete — all six code sub-phases (26a–26f) implemented and tested; full suite 682 passing (31 new tests). The one-time manual exchange cleanup of the orphaned ETH-1750 inventory (26f) is the only outstanding item. Config gains `MIN_NEAR_DTE_AT_ENTRY`, `SPREAD_VALUE_BASIS`, `CLOSE_PROCEEDS_WARN_PCT`, and `RECONCILE_ESCALATE_AFTER_CYCLES` (exec basis enabled in `config_test.py`). Offline demos: `python -m scratch.scratch_partial_fill_flatten` and `python -m scratch.scratch_roll_resilience` (no live orders). Surfaced by analysis of the 2026-07-22 → 2026-07-23 test-mode run (trades 14–18, `logs/bot_test.log`, `db/calendar_bot_test.db`) and the 2026-07-22 paper-mode close of trade 208 (`logs/bot.log`, `db/calendar_bot.db`). Headline damage: three healthy test positions (110%/101%/98% of debit at the decision tick) were force-liquidated on a single failed roll attempt for −162.42/−61.59/−89.65; paper trade 208 was closed the same way after spot drifted its strike past the moneyness cap; deep-ITM trade 17 entered at 14.77% moneyness (just under the 15% cap) and stopped out at "0% of debit" three minutes later for −100.80; and three failed legged entries on ETH 1750 left **untracked exchange inventory** — 13 naked short 24JUL26-1750-P plus an unrecorded 5×5 24JUL/7AUG calendar — that the DB knows nothing about (`RECONCILE MISMATCH` warned every 5 minutes for 30+ hours with no escalation).

**Implementation summary:** 26a — `execution/executor._cancel_and_flatten` reads `filled_amount` after every timeout-cancel (entry near/far, close near/far, roll close/open) and submits a reverse order for the filled portion at a crossed price; `order_manager` gains a `CANCELLED_PARTIAL` terminal state + `filled_amount`; a failed flatten logs `CRITICAL` and fires a one-shot alert via the notifier now threaded into `CalendarExecutor`. 26b — `_check_slippage(side=…)` rejects only adverse fills; an adverse far-leg fill after both legs filled unwinds both legs before raising. 26c — a failed roll holds the position and retries up to `POSITION_FAILURE_RETRY_CAP`; `_try_roll` iterates all `scan(roll_for=…)` candidates (excluding the held near), with moneyness/POP/contango relaxed for rolls but liquidity/OI/gap/margin retained; roll-fail and drain closes use a `close_label` override (`Closed (Roll Failed)` / `Closed (Drain)`). 26d — `_eval_candidate` rejects entries whose matched near DTE < `MIN_NEAR_DTE_AT_ENTRY` (skipped for rolls). 26e — `_get_spread_values` returns mark + executable (`far_bid − near_ask`) values, `SPREAD_VALUE_BASIS` selects the decision basis, both logged (`sv_mark`/`sv_exec`), a pre-close `WARNING` fires when executable proceeds < `CLOSE_PROCEEDS_WARN_PCT` of mark, and `DryRunExecutor.close_spread` returns `last_spread_value` so paper closes realise MTM P&L. 26f — `PortfolioTracker._reconcile` escalates a persistent identical mismatch fingerprint to a one-shot alert after `RECONCILE_ESCALATE_AFTER_CYCLES` cycles.
...
2. `_try_roll` iterates **all** matches (skipping any whose near equals the held near instrument) instead of only `matches[0]`, and the roll search relaxes *entry-grade* filters that make no sense for keeping an existing position alive: moneyness, POP, and contango are not re-applied to the position's own strike (documented decision, operator-reviewed and approved 2026-07-23 — they gate new risk, not the continuation of existing risk; a drifted position is kept alive at roll time and exits via its stop/TP, not via entry filters); the two-sided-quote, OI-on-the-new-near, liquidity-gate, `MIN_ROLL_NEAR_FAR_GAP_DAYS`, and margin-gate checks all remain. Implemented as a `roll_for=pos` mode of `scan()` (or a dedicated `scan_roll_candidates()`), constrained to the position's strike/type/far leg.
