...
Planned — not yet implemented. Root-cause detail above is grounded in direct code reading (`db/state.py`, `strategy/decision.py`, `execution/executor.py`, `telegram_cmd/handlers.py`, `telegram_cmd/listener.py`, `config.py`) plus the two reported Telegram screenshots (trades #11/#12, test mode) and the live paper-mode DB rows for trades #206/#207. No code has been changed yet; this phase is scoped so 22a/22b/22c (visibility + silent-failure hardening) can ship independently of 22d (price-rejection root cause) and 22f (roll-trigger/roll-matching fix), since they touch different files and fix different symptoms of the same underlying "positions get stuck and then vanish" problem.
