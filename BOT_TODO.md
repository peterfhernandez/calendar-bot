...
**Status: not started.** Found via analysis of `db/calendar_bot_test.db` and `logs/bot_test.log*` on 2026-07-07 (test-mode run under `config_test.py`, 2026-06-28 → 2026-07-07). See BOT_PLAN.md Phase 18 for full root-cause writeups. Three distinct bugs, all in the close/retry path, compounded to leave losing positions open far longer than their configured stop-loss, spam duplicate stuck-state DB writes, and in one case record a $0.00 PnL for a trade that actually lost money.

**Observed impact:** of 4 closed trades in the test DB, 3 went through an abnormal close (retry-failed / stuck / manual). One position (trade_id=3) sat in a failing retry loop for ~29 hours and was re-marked "stuck" 40 separate times; another (trade_id=5) required a manual `/close_manually` after 6+ days stuck. `Deribit error -32602: Invalid params` on far-leg close orders occurred 636 times across the log history.

### Bug 1 — Far-leg close order rejected by Deribit (`-32602 Invalid params`)

- [ ] **Root cause:** `execution/executor.py` never fetches an instrument's actual tick size (Deribit's option tick size scales with premium level via `tick_size_steps`). Every order price — entry, close, roll, unwind — is blanket-rounded with `round(price, 4)` (see `_index_price`, and the price args in `_async_close_spread`, `_async_enter_spread`, `_async_roll_near_leg`). Near-leg close orders happen to land on valid ticks and never fail (0 failures logged); far-leg close orders are typically priced higher (more time value) and fall in a coarser tick band, so the 4-decimal price is invalid and Deribit's JSON-RPC layer rejects the request before it reaches business-logic validation — hence the raw `-32602` code rather than a Deribit-specific error like `10009`/`11044`.
- [ ] **Fix:** add a helper that fetches (and caches, per instrument) `tick_size` / `tick_size_steps` via `public/get_instruments` or the ticker response, and round every order price to the nearest valid tick (floor for buys, ceiling for sells, or per Deribit convention) before submission. Apply to **all** price-producing call sites in `executor.py`: `_async_enter_spread` (individual-leg fallback), `_async_enter_spread_combo`, `_async_close_spread` (near + far), `_async_roll_near_leg` (close + open), and the near-leg unwind/flatten paths (`FLATTEN-NEAR`, `UNWIND-NEAR`, `UNWIND-FAR`).
- [ ] **Test coverage:** new tests in `tests/test_executor.py` asserting (a) a price that doesn't align to a mocked coarse tick size gets rounded before submission, (b) near and far legs each use their own instrument's tick size rather than a shared constant, (c) a regression test replaying the exact instrument/price pair from trade_id=3/4/5's logs no longer raises.
- [ ] **Scratch verification:** `scratch/scratch_tick_size_close.py` — connects to test.deribit.com, fetches a real far-dated option's tick size, and confirms a manually constructed close order at the corrected price is accepted (paper-safe: read-only ticker calls only, no live order unless `TRADING_MODE` explicitly allows it per existing scratch conventions).

### Bug 2 — Stuck-position retry counter reset allows an unbounded retry loop

- [ ] **Root cause:** in `strategy/decision.py`, the stop-loss/take-profit/near-leg-expired "mark as stuck" branches (~lines 750–778, 818–848) call `mark_position_close_stuck()` and then immediately `self._close_roll_failures.pop(trade_id, None)`. This clears the in-memory failure counter back to zero. Separately, `mark_position_close_stuck()` only ever updates the `close_status` column in `db/state.py` — it never changes `result`, and `get_open_trades()` (the query the monitor loop uses every tick) filters solely on `result`, not `close_status`. So a "stuck" position is never actually excluded from monitoring: it keeps getting re-evaluated every tick, the same close order keeps failing (Bug 1), the counter climbs 0→1→2→3 again, and the position gets re-marked "stuck" (redundant DB write + ERROR log line) over and over — 40 times for trade_id=3 over 29 hours — until the market moves the spread back above the trigger threshold or a human intervenes. The existing test `test_fourth_failure_force_closes_position` (`tests/test_decision.py`) currently *asserts* the counter is cleared after marking stuck, i.e. the test encodes this bug as expected behavior rather than catching it.
- [ ] **Fix:** decide and implement one of: (a) exclude `close_status == 'close_stuck'` positions from the monitor loop's stop/tp/roll re-evaluation entirely until a human clears them via `/close` or `/close_manually` (simplest, matches the "needs manual intervention" messaging already sent to the user), or (b) keep retrying but on a much slower cadence (e.g. once per `SCAN_INTERVAL_SEC` instead of every `MONITOR_INTERVAL_SEC`) and track "already stuck" as separate state from the retry counter so it can't silently reset itself into a fast loop. Option (a) is recommended given Bug 1's fix should make most closes succeed anyway, and repeated auto-retry against an exchange that already rejected the request is unlikely to be productive.
- [ ] **DB-level fix:** either add `close_status` to the `WHERE` clause of `get_open_trades()` (or a new query variant used specifically by the monitor loop) so stuck positions stop being fetched as ordinary open positions, or introduce a distinct `result` value for stuck positions that the existing open/closed filters already understand.
- [ ] **Update existing tests:** `test_fourth_failure_force_closes_position` and `test_fourth_stop_close_failure_force_closes` in `tests/test_decision.py` need to be rewritten to assert the *new* intended behavior (counter frozen / position excluded from re-evaluation) rather than the current reset-to-zero behavior.
- [ ] **New test:** simulate 10+ consecutive monitor ticks against a permanently-failing executor and assert `mark_position_close_stuck` / `notify_close_stuck` are each called at most once, and the position is not re-evaluated for stop/tp/roll after being marked stuck.

### Bug 3 — Force-closed position recorded with `pnl=0.0` instead of real P&L

- [ ] **Root cause:** trade_id=1 in `calendar_bot_test.db` closed with `result='Loss (Stop retry limit exceeded)'` and `pnl=0.0`, despite `last_spread_value=57.13` sitting close to (but below) `net_debit=57.42` — a real, non-zero loss. `_close_position()` in `strategy/decision.py` only writes a result when `self._executor.close_spread(pos)` returns a non-`None` credit; when it returns `None` the function returns early with `"...close FAILED"` and never calls `close_calendar_trade()` — so this specific DB row must have been written by an older/removed force-close code path (see `git log -- strategy/decision.py`: `740895e`, `d0cc03a`, `4f4c2b6`, `f5dc9c9` all touched this retry logic). Whatever path produced it evidently recorded a result/PnL pair without a valid `close_credit`, defaulting PnL to 0.
- [ ] **Fix:** audit every code path that can call `close_calendar_trade()` (direct force-close, expired-near-leg force-close, roll-retry-limit-exceeded force-close) and ensure each one computes PnL from the last known mark-to-market value (`last_spread_value`, or a fresh `check_calendar_status` call) rather than defaulting to 0 when `close_spread()` fails or returns `None`. If a position must be force-closed without a confirmed exchange fill, the recorded PnL should be the best available estimate (last known spread value minus debit), clearly flagged (e.g. `close_error_reason` already supports this) rather than silently zeroed.
- [ ] **New test:** assert that no code path can call `close_calendar_trade()` with `pnl=0.0` when `last_spread_value` (or an equivalent mark-to-market input) indicates a non-zero gain/loss; add a regression test replaying trade_id=1's exact inputs.
